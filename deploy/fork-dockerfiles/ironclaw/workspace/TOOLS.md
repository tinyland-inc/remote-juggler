...
- **GitHub token**: Automatically resolved by gateway. No need to pass tokens explicitly
...
- **NUL bytes**: Chapel binary can emit NUL bytes in output. Gateway strips them automatically
...
# Fetch a file from GitHub
github_fetch(owner="tinyland-inc", repo="ironclaw", path="package.json")

# Store campaign results
juggler_setec_put(name="campaigns/oc-dep-audit", value="{...}")

# Check identity status
juggler_status()

# Validate bot identity connectivity
juggler_validate(identity="rj-agent-bot")

# Request a missing secret
juggler_request_secret(name="brave-api-key", reason="Web search for upstream monitoring", urgency="medium")
```
